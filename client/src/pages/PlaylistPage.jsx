...
const PlaylistPage = () => {
  ...
  useEffect(() => {
    ...
    const loadPlaylists = async () => {
      try {
        ...
        setStatus({
          loading: false,
          error: requestError.response?.data?.message || "Could not load playlists.",
          success: "",
        });
      }
    };
  ...
  const handleCreate = async (event) => {
    ...
    try {
      ...
      setStatus((current) => ({
        ...current,
        error: error.response?.data?.message || "Could not create the playlist.",
        success: "",
      }));
    }
  };
  ...
  return (
    <div className="space-y-8">
      <SectionTitle
        ...
        description="Create curated sets and manage them from one place. Add or remove videos directly from the player page."
      />

      <section className="grid gap-6 lg:grid-cols-[1fr,1.2fr]">
        <div className="space-y-5">
          <form className="glass-panel grid gap-4 p-6" onSubmit={handleCreate}>
            <h3 className="text-xl font-bold text-on-surface">Create a playlist</h3>
            ...
            {status.success ? (
              <p className="text-sm text-accent">{status.success}</p>
            ) : null}
          ...
          {status.loading ? (
            <div className="glass-panel p-6 text-on-surface-variant">
              Loading playlists...
            </div>
          ) : null}
        ...
        <div className="space-y-4">
          <h3 className="text-xl font-bold text-on-surface">Selected playlist</h3>
          {!selectedPlaylist ? (
            <EmptyState
            ...
            <>
              <div className="glass-panel p-6">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="pill">{selectedPlaylist.videos.length} saved</span>
                </div>
              </div>
